...
bool update_telemetry(struct telemetry_data_t *telemetry, uint32_t eva_time, struct backend_data_t *backend, bool isEVA1)
{
    ...
    if (uia_power_supply_connected && dcu_using_umbilical_power)
    {
        ...
        {
            telemetry->batt_time = BATT_TIME_CAP;
        }
    ...
    { //----------------------------- Outside of HAB on EVA Mission
        ...
        if (dcu_using_pri_oxy)
        {
            ...
            if (telemetry->oxy_pri_tank_fill < 0)
            {
        ...
        else
        {
            ...
            if (telemetry->oxy_sec_tank_fill < 0)
            {
    ...
    if (telemetry->oxy_pri_tank_pressure < 0.001 || telemetry->oxy_pri_tank_fill < 0.001f)
    {
        telemetry->oxy_pri_tank_pressure = 0.0f;
    }
    telemetry->oxy_sec_tank_pressure += randomized_sine_value(x, 0, 0.01f, 1260.0f, 0.001f);
    if (telemetry->oxy_sec_tank_pressure < 0.001 || telemetry->oxy_sec_tank_fill < 0.001f)
    {
        telemetry->oxy_sec_tank_pressure = 0.0f;
    }
...
void simulate_backend(struct backend_data_t *backend)
{
    ...
    double roverpos[2] = {1600.0, 1600.0};
    ...
    if (time_incremented)
    {
        ...
        for (int i = 0; i < NUMBER_OF_TEAMS; i++)
        {
            ...
            if (eva->started && !eva->completed && !eva->paused)
            {
                ...
                roverpos[0] = 80 + 5 * cos(eva->total_time / 20.0) + random_normal(0, 0.5);
                roverpos[1] = 80 + 5 * sin(eva->total_time / 20.0) + random_normal(0, 0.5);
                ...
                backend->rover.pos_x = roverpos[0] + top_left_easting;
                backend->rover.pos_y = top_left_northing - roverpos[1];
                ...
                update_imu("eva2_posx", &backend->imu);
                update_imu("eva2_posy", &backend->imu);
                update_rover("pos_x", &backend->rover);
                update_rover("pos_y", &backend->rover);
